...
export const decisionSchema = z.object({
  ...
  url: z.string().trim().max(shortField).url().or(z.literal("")).default(""),
  desiredOutcome: z.string().trim().max(INPUT_LIMITS.desiredOutcome).default(""),
});
...
export const evaluationResponseSchema = z.object({
  ...
  alignmentLabel: z.enum(ALIGNMENT_LABEL_VALUES),
